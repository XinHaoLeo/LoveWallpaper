...
object DaoUtils {
    ...
    //暂时没做账号登陆,所以默认一个账户
    private const val mUserName = "Leo"
    private var mOnNotifyDataChangedListener: OnNotifyDataChangedListener? = null
    ...
    fun addRecords(record: String) {
        ...
        try {
            ...
            mOnNotifyDataChangedListener?.notifyDataChanged()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
     ...
     * 清除指定用户的搜索记录
     */
    fun deleteUsernameAllRecords() {
        try {
            mSearchRecordDb = getWritableDatabase()
            mSearchRecordDb.delete(TABLE_NAME, "username = ?", arrayOf(mUserName))
            mOnNotifyDataChangedListener?.notifyDataChanged()
